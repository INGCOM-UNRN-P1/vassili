# Manual de Uso y Referencia Técnica: vassili

> **VASSILI** — Motor de Mutation Testing en C para evaluación de calidad y robustez de suites de tests
> **Versión:** `0.1.0` · **CLI principal:** `vassili` · **Plugin Ripley:** `mutation_testing`

---

## 1. Arquitectura y Propósito Pedagógico

`vassili` forma parte del ecosistema de herramientas de la cátedra de Programación 1 (UNRN). Su objetivo central es resolver de forma modular, determinista y automatizada las tareas asociadas a su dominio específico dentro del ciclo de desarrollo, evaluación y aprendizaje de software en C.

### Alcance Funcional (Qué cubre)
- Motor de Mutation Testing para código fuente C y evaluación de calidad de suites de pruebas.
- Aplicación de operadores de mutación por **sustitución textual** línea a línea (no sobre un AST): inversión de comparaciones, mutación de operadores aritméticos y cambio de conectores lógicos. Antes de mutar se enmascaran comentarios y literales de cadena/carácter, así que un `+` dentro de un `"..."` no se toca. No muta literales numéricos ni constantes.
- Ejecución de la suite de pruebas contra cada mutante generado.
- Cálculo cuantitativo del Mutation Score (porcentaje de mutantes eliminados respecto al total) y reporte de mutantes supervivientes.

### Límites de Responsabilidad y Delegación (Qué no cubre)
- Cobertura lógica estructural MC/DC (delegado a `dietrich`).
- Fuzzing de entradas extremas de I/O (delegado a `drake`).
- Orquestación de calificaciones masivas de cursos (delegado a `dredd`).

### Principios de Diseño
- **Enfoque Pedagógico:** Diagnósticos y mensajes en español rioplatense orientados a facilitar la comprensión de errores conceptuales.
- **Salida Estructurada Dual:** Soporte nativo para visualización enriquecida en terminal (Rich) y salida parseable para orquestadores (`--json`).
- **Integración Contractual:** Capacidad de emitir secciones de reporte para `dredd` (`dredd-section`) y actuar como satélite orquestado por `ripley`.
- **Idempotencia y Robustez:** Validación de precondiciones y comandos de autodiagnóstico (`doctor`) para verificación del entorno.

---

## 2. Instalación y Requisitos

### Requisitos del Sistema
- **Python:** `>= 3.10` (recomendado Python 3.11 o 3.12).
- **Gestor de paquetes:** [`uv`](https://github.com/astral-sh/uv) (entorno estándar de cátedra).
- **Toolchain C (si aplica):** GCC / Clang, Make, GDB y bibliotecas estándar de desarrollo.

### Instalación en el Entorno de Usuario
Para instalar la herramienta de forma global y aislada en el sistema mediante `uv tool`:
```bash
uv tool install --editable /home/mrtin/dev/tools/vassili
```

### Verificación de Instalación
Ejecutá el comando `doctor` para constatar que todas las dependencias y binarios requeridos estén presentes y operativos:
```bash
vassili doctor
```

---

## 3. Guía Integral de Comandos (CLI)

| Comando | Descripción Breve |
| :--- | :--- |
| [`vassili check`](#check) | Genera mutantes sintéticos del código C y evalúa qué porcentaje es detectado por los tests. |
| [`vassili mutate`](#mutate) | Genera mutantes sintéticos del código C y evalúa qué porcentaje es detectado por los tests. |
| [`vassili report`](#report) | Genera directamente la sección de reporte Markdown de VASSILI para Dredd. |
| [`vassili version`](#version) | Muestra la versión de VASSILI. |
| [`vassili doctor`](#doctor) | Verifica el estado del entorno de VASSILI (Python, GCC). |

### `vassili check`

Genera mutantes sintéticos del código C y evalúa qué porcentaje es detectado por los tests.

#### Argumentos
| Argumento | Tipo | Descripción |
| :--- | :--- | :--- |
| `source_file` | `<class 'pathlib._local.Path'>` | Archivo C a mutar |

#### Opciones y Banderas
| Opción / Banderas | Tipo | Por Defecto | Descripción |
| :--- | :--- | :--- | :--- |
| `--tests-dir`, `-t` | `<class 'pathlib._local.Path'>` | `tests` | Directorio con casos de prueba .in/.out |
| `--min-score`, `-m` | `<class 'float'>` | `70.0` | Score mínimo de mutación para aprobar (0-100%) |
| `--timeout` | `<class 'float'>` | `2.0` | Timeout máximo por test en segundos |
| `--json` | `<class 'bool'>` | `False` | Emitir salida en formato JSON estructurado |
| `--md`, `--output-md` | `Optional[pathlib._local.Path]` | `None` | Generar sección de reporte en formato Markdown para fusión en Dredd. |

#### Ejemplo de Invocación
```bash
vassili check <source_file>
```

### `vassili mutate`

Genera mutantes sintéticos del código C y evalúa qué porcentaje es detectado por los tests.

#### Argumentos
| Argumento | Tipo | Descripción |
| :--- | :--- | :--- |
| `source_file` | `<class 'pathlib._local.Path'>` | Archivo C a mutar |

#### Opciones y Banderas
| Opción / Banderas | Tipo | Por Defecto | Descripción |
| :--- | :--- | :--- | :--- |
| `--tests-dir`, `-t` | `<class 'pathlib._local.Path'>` | `tests` | Directorio con casos de prueba .in/.out |
| `--min-score`, `-m` | `<class 'float'>` | `70.0` | Score mínimo de mutación para aprobar (0-100%) |
| `--timeout` | `<class 'float'>` | `2.0` | Timeout máximo por test en segundos |
| `--json` | `<class 'bool'>` | `False` | Emitir salida en formato JSON estructurado |
| `--md`, `--output-md` | `Optional[pathlib._local.Path]` | `None` | Generar sección de reporte en formato Markdown para fusión en Dredd. |

#### Ejemplo de Invocación
```bash
vassili mutate <source_file>
```

### `vassili report`

Genera directamente la sección de reporte Markdown de VASSILI para Dredd.

#### Argumentos
| Argumento | Tipo | Descripción |
| :--- | :--- | :--- |
| `source_file` | `<class 'pathlib._local.Path'>` | Archivo C a mutar |

#### Opciones y Banderas
| Opción / Banderas | Tipo | Por Defecto | Descripción |
| :--- | :--- | :--- | :--- |
| `--output`, `-o` | `Optional[pathlib._local.Path]` | `None` | Ruta de destino del archivo Markdown. |
| `--tests-dir`, `-t` | `<class 'pathlib._local.Path'>` | `tests` | Directorio con casos de prueba. |

#### Ejemplo de Invocación
```bash
vassili report <source_file>
```

### `vassili version`

Muestra la versión de VASSILI.

#### Ejemplo de Invocación
```bash
vassili version
```

### `vassili doctor`

Verifica el estado del entorno de VASSILI (Python, GCC).

#### Opciones y Banderas
| Opción / Banderas | Tipo | Por Defecto | Descripción |
| :--- | :--- | :--- | :--- |
| `--json` | `<class 'bool'>` | `False` | Emitir diagnóstico en formato JSON estructurado. |

#### Ejemplo de Invocación
```bash
vassili doctor
```

---

## 4. Formatos de Salida e Integración con el Ecosistema

### Modo Interactivo / Terminal (Rich)
Por defecto, la herramienta renderiza paneles, árboles y tablas estilizadas para facilitar la lectura del estudiante y docente en terminales modernas con soporte ANSI.

### Modo Estructurado JSON (`--json`)
Para integración con pipelines de CI/CD, scripts de automatización u orquestadores externos, la opción `--json` emite un documento JSON estricto por la salida estándar (`stdout`), dirigiendo cualquier mensaje de logging a `stderr`:
```bash
vassili check --json
```

### Integración con Dredd (`dredd-section`)
Cuando la herramienta genera reportes de evaluación para entregas de alumnos, produce una sección Markdown estandarizada conforme al contrato de integración de Dredd (v1.0.0):
```markdown
<!-- dredd-section: vassili, tool=vassili, version=0.1.0, status=ok -->
```
Este encabezado garantiza la agregación determinista de los hallazgos en la rúbrica docente.

### Integración con Ripley
`vassili` está registrada en el catálogo de plugins satélites de Ripley (`SATELLITE_CATALOG`). Puede invocarse directamente a través del motor de evaluación de Ripley configurando el análisis en `ripley.toml`.

---

## 5. Diagnóstico y Códigos de Salida

### Códigos de Retorno (`exit code`)
| Código | Significado |
| :---: | :--- |
| `0` | Ejecución exitosa sin hallazgos críticos ni errores de sintaxis. |
| `1` | Hallazgos pedagógicos detectados, infracción de reglas o advertencias activas. |
| `2` | Error de sintaxis en argumentos CLI o archivo fuente no encontrado. |
| `>2` | Error no recuperable del sistema, fallo de memoria o excepción interna. |

### Diagnóstico del Entorno (`doctor`)
Ante comportamientos inesperados, verificá el estado operativo con:
```bash
vassili doctor
```
Comprueba la presencia de las dependencias requeridas y la integridad de los componentes del paquete.