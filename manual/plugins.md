---
title: "Guía de Extensión y Creación de Plugins: vassili"
subtitle: "Manual de integración, desarrollo de extensiones y uso de la API Python de vassili"
author: "Cátedra de Algoritmos y Programación"
date: "2026-08-31"
---

(manual-vassili-plugins)=
# Guía de Extensión y Plugins: vassili

````{abstract}
Esta guía técnica detalla cómo desarrollar extensiones, crear nuevos plugins e integrar programáticamente **`vassili`** en herramientas de evaluación, entornos de integración continua (CI/CD) o scripts docentes.
````

---

(manual-vassili-plugins-arquitectura)=
## 1. Arquitectura de Extensión

`vassili` provee una arquitectura modular desacoplada basada en puntos de entrada (Entry Points) estándar de Python (`[project.entry-points]`) o interfaces de inyección funcional:

- **Mecanismo de Extensión Principal**: `Extensiones y Reglas Personalizadas para vassili`.
- **Punto de Entrada Oficial**: `vassili.plugins`.
- **Formato de Comunicación**: Estructuras de datos serializables JSON / Pydantic models.

---

(manual-vassili-plugins-tutorial)=
...
class CustomPlugin:
    """Plugin de extensión para vassili."""
    name = "custom_rule"
...
Para que `vassili` descubra y cargue automáticamente tu plugin, agregalo en tu `pyproject.toml`:

````{code-block} toml
[project.entry-points."vassili.plugins"]
mi_plugin = "mi_paquete.modulo:MiPlugin"
````

### Paso 3: Instalar y Verificar el Plugin

Instalá tu extensión en modo editable y comprobá que `vassili` la reconozca:
...
vassili plugins list
````

---

(manual-vassili-plugins-sdk)=
## 3. Conexión Programática mediante la API Python

Podés importar y ejecutar `vassili` directamente desde scripts de Python sin invocar subprocesos:
...
resultado = vassili.ejecutar_analisis(
...
(manual-vassili-plugins-ci)=
## 4. Integración en Pipelines de CI/CD (GitHub Actions / GitLab CI)

Podés integrar `vassili` en tus flujos automatizados de Git para bloquear entregas que no cumplan los requisitos de cátedra:
...
jobs:
  auditoria:
    ...
    steps:
      ...
      - name: Instalar vassili
        run: pip install -e ./vassili
        
      - name: Ejecutar Auditoría
        run: vassili check src/ include/ --json > reporte.json
````

---

(manual-vassili-plugins-ejercicios)=
